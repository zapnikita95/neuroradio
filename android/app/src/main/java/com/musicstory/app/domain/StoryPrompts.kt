...
""".trimIndent()
    ...
    fun userMessage(
        ...
        return buildString {
            ...
            appendLine(
                "ЗАПРЕЩЕНО выдумывать социальные темы (расизм, дискриминация, равенство) — только если это ЕСТЬ в семени.",
            )
            ...
            if (selectedFact != null) {
                ...
                appendLine()
                appendLine("РЕЦЕПТ: 1) крючок = контраст/парадокс из семени 2) деталь 3) финал одной фразой из семени.")
                appendLine("ЗАПРЕЩЕНО: «разорвал кабину», «разорвёт кабину» — служебная метафора, не писать в script.")
