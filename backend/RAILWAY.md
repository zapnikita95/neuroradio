...
**Telegram (как Movie Planner):** без `/setdomain` — приложение открывает `t.me/БОТ?start=mobileauth_КОД`, бот на Railway подтверждает вход.

| `TELEGRAM_BOT_TOKEN` | Токен от @BotFather |
| `TELEGRAM_BOT_USERNAME` | Username **без @** — тот же бот, что отвечает на /start |

Бот должен быть **живой**: BFF на Railway сам слушает Telegram (`getUpdates`). В логах после деплоя: `[telegram-bot] long polling started`.
