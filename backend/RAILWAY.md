...
**iOS озвучка:** AVPlayer **не играет OGG**. Для iOS клиент шлёт `client_platform: ios` → бэкенд отдаёт **WAV**. Подробно: `docs/IOS_AUDIO_PLAYBACK.md`. Не менять глобально на OGG «как Android».

**Telegram OAuth (iOS, как Movie Planner):** системный браузер `oauth.telegram.org` → **не** Login Widget и **не** `/setdomain`.

| Variable | Зачем |
|----------|-------|
| `TELEGRAM_BOT_TOKEN` | Токен `@neuroradio_auth_bot` |
| `TELEGRAM_BOT_USERNAME` | `neuroradio_auth_bot` (без @) |
| `TELEGRAM_OIDC_CLIENT_SECRET` | **Обязательно.** BotFather → бот → **Web Login** → **Client secret** |
| `TELEGRAM_OAUTH_REDIRECT_URI` | `https://www.efir-ai.ru/v1/public/oauth/telegram/callback-bridge` (в BotFather → OAuth redirect URLs) |

Проверка после деплоя:
```bash
curl -s https://www.efir-ai.ru/v1/public/auth-config | jq '.telegramOAuthEnabled,.telegramBotId,.telegramOAuthRedirectUri'
# telegramOAuthEnabled: true
curl -s 'https://www.efir-ai.ru/v1/public/oauth/telegram/callback-bridge?code=x' | head -3
# HTML «Возвращаем в приложение…»
```

Логи Railway при входе: `[telegram-oauth] token exchange`, `callback-bridge`, `POST /telegram/oauth`.

**Telegram Login Widget (legacy WebView):** только если нужен старый путь.

| `TELEGRAM_WIDGET_BASE_URL` | `https://www.efir-ai.ru` |
...
5. @BotFather → `/setdomain` → бот **`neuroradio_auth_bot`** → **`efir-ai.ru`** (apex, **не** `www.efir-ai.ru`)

Если в BotFather указан `www.efir-ai.ru`, задай на Railway `TELEGRAM_WIDGET_KEEP_WWW=true`.

Проверка API: `curl -s https://www.efir-ai.ru/v1/public/auth-config` → `telegramWidgetBaseUrl` должен совпадать с доменом в BotFather.
...
### Release / Play Store — авторизация

**Play App signing SHA-256 уже в коде бэкенда** (`jwt.ts`) — Railway переменная **не обязательна**.

Текущий ключ (из `deployment_cert.der` / Play Console → Подписание приложений):

```
5F:45:43:50:D5:4E:04:8E:F7:ED:C7:B6:18:08:33:7D:D1:BF:91:D9:4B:3C:CF:E7:50:43:A7:A5:AF:A7:F0:B5
```

Upload keystore (sideload release APK) — тоже в коде:

```
6C:2A:59:AB:FB:AC:C6:B8:28:D4:C0:32:1B:E5:F8:48:05:69:88:55:86:77:E4:12:3D:21:62:00:C5:31:B0:09
```

Если Google **сменит** App signing key — скачай новый `deployment_cert.der`, обнови `PLAY_APP_SIGNING_CERT_SHA256` в `jwt.ts` или добавь в Railway:

```
ALLOWED_CERT_SHA256=новый_sha256
```
...
### Тестовые учётки для проверяющих (ЮKassa / App Store)
