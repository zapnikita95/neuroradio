...
| `ACCOUNT_DATA_DIR` | Volume `/data` — **facts-bank.json** (кэш фактов), опционально |
| `DATABASE_URL` | **Railway Postgres** — аккаунты, auth/trial, feedback, fact-misses, dev-tier (авто-импорт из JSON при первом старте) |

**Volume + Postgres (рекомендуется):**
- **Postgres** — `DATABASE_URL` из Railway Postgres plugin (accounts, email codes, usedSeeds, story feedback)
- **Volume** `/data` + `ACCOUNT_DATA_DIR=/data` — только `facts-bank.json` (тяжёлый кэш, можно оставить на томе)
