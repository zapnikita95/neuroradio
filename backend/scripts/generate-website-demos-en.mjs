...
const PERSONAS = [
  {
    ...
    voice: 'charlie',
    speed: 0.94,
    studioVoices: ['charlie', 'adam', 'josh'],
    short:
    ...
    minute: THRILLER_STORY_MINUTE + ' Volume up — this is the one that hijacked MTV!',
    full: THRILLER_STORY_FULL + ' Volume up — this is the one that hijacked MTV!',
  },
  {
    id: 'night_dj',
    voice: 'antoni',
    speed: 0.84,
    studioVoices: ['antoni', 'adam'],
  ...
  },
  {
    ...
    voice: 'josh',
    speed: 0.96,
    studioVoices: ['josh', 'adam', 'charlie'],
...
async function writePreview() {
  ...
  const out = {
    ...
    studioLong: PERSONAS.map((p) => ({
      persona: p.id,
      voice: p.voice,
      len2: { speakable: p.minute, display: p.minute, file: studioLongFile(p.id, '-len2', '.ogg') },
      len4: { speakable: p.full, display: p.full, file: studioLongFile(p.id, '-len4', '.ogg') },
    })),
  };
