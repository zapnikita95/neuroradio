...
export const STORY_NARRATOR_PRESETS: Record<Exclude<StoryNarratorId, 'auto'>, StoryNarratorPreset> = {
  ...
  contemporary: {
    ...
    speechStyle:
      'живая речь эпохи: «тогда», «в те годы», запах улицы, звук радио — но факт из ОПОРНЫЕ ФАКТЫ',
    contentFocus:
      'проверяемый факт из ОПОРНЫЕ ФАКТЫ, поданный голосом человека той эпохи; не выдумывай «я был в клубе»',
    formatRules:
      'Можно первое лицо только как подача факта. Не обращайся к слушателю как ведущий. Начни с факта, не с «помню».',
  },
  expert: {
    id: 'expert',
