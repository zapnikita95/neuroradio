...
import { fixWikiTranslationArtifacts, findWikiTranslationDefects } from './wiki-translate-quality.js';
...
- Имена артистов, названия треков, альбомов, лейблов — ТОЧНО как в оригинале (латиницей). НИКОГДА не переводи название трека на русский. Не ломай «Gorillaz», «De La Soul», «Damon Albarn» — только латиница, без «Горил Laz» и без «Дамон Албарн».
- ЗАПРЕЩЕНО: «этот артист», «этот исполнитель», «эта группа» вместо имени; «— в треке» без названия; символ + внутри слов.
- Если указан псевдоним — назови его латиницей (Fatboy Slim, Stromae), не «этот артист».
- «comic strips» → «комиксы» или «комиксы-стрипы», НИКОГДА «стрипты» и «каракаты».
- Без «уникальный», «легенда», «магия музыки», без воды.
- ГОДА И ЦИФРЫ ДЛЯ ОЗВУЧКИ: не перечисляй годы альбомов, сериалов, релизов списком. Максимум один временной ориентир словами («в те годы», «к началу двадцатых», «недавно»). Без скобок (2014–2017), без «2015, 2016, 2018». Если в оригинале много дат — выбери один главный факт без дат или одну дату словами.
- Без арабских цифр в script.
- 40–95 слов, 2–4 предложения, связный рассказ.
- Если указан трек — упомяни его название в конце одной фразой (оригинал, без перевода).
JSON: {"script":"..."}`;
...
function scriptFromRaw(raw: string, artist: string, title: string, wikiLead: string): string | null {
  ...
  const wikiDefect = findWikiTranslationDefects(fixed, artist, title);
  if (wikiDefect) return null;
  const quality = validateStoryScript(fixed, DEFAULT_STORY_LENGTH, artist, title, {
...
export async function translateWikiLeadToStory(input: IndieWikiStoryInput): Promise<string | null> {
  if (input.wikiLang === 'ru') {
    ...
    if (factMentionsArtist(cleaned, input.artist) && cleaned.length >= 35) {
      const fixed = preserveMusicProperNames(cleaned, input.artist, input.title);
      if (findWikiTranslationDefects(fixed, input.artist, input.title)) return null;
      return stripBannedFluff(fixed);
    }
  }
