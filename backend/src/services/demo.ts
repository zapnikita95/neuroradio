...
function buildAngleScript(
  ...
  const templates = [
    `Помню студию — при записи «${title}» ${role} настояли оставить дубль с ошибкой. Тот срыв голоса стал фирменным моментом, инженер потом говорил, что микрофон еле остыл.`,
