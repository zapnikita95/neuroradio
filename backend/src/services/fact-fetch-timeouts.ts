 ...
 */
export const FACT_FETCH_BUDGET_MS = parseInt(process.env.FACT_FETCH_TIMEOUT_MS ?? '28000', 10);

/** Default per-source wall cap in parallel harvest (Promise.all waits for slowest). */
export const FACT_SOURCE_CAP_MS = parseInt(process.env.FACT_SOURCE_CAP_MS ?? '22000', 10);
...
export const FACT_DEDICATED_SOURCE_TIMEOUT_MS = parseInt(
  process.env.FACT_DEDICATED_SOURCE_TIMEOUT_MS ?? '15000',
  10,
);
