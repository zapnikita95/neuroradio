...
const LLM_GARBAGE_PATTERNS: RegExp[] = [
  ...
  /\b(?:я|мы)\s+(?:слышал[аи]?|слышали),?\s+как\b/i,
  /\bмне\s+(?:рассказывал[аи]?|говорил[аи]?),?\s+что\b/i,
  /\bстал\s+[а-яё]*\s*хитом[^.]{0,55}\bв\s+памят/i,
  /\bхитом\s+[^.]{0,45}\bв\s+памят/i,
  /\bне\s+только\s+в\s+чарте[^.]{0,45}\bв\s+памят/i,
  /\bя\s+(?:вложил|вложила|заплатил|заплатила|инвестировал|инвестировала)\b[^.]{0,70}(?:миллион|тысяч|полмиллион|сот\s+тысяч|доллар)/i,
