...
- Латиница в тексте — только: имя артиста, название трека в «кавычках», устоявшиеся термины (moonwalk, anti-gravity lean, Billboard). Иначе — БРАК, перепиши.
...
ЯЗЫК: русский. Английский/latin — имя артиста, «название трека», устоявшиеся термины (moonwalk и т.п.).
...
export function buildStoryUserPrompt(params: {
  ...
  if (selected) {
    ...
    lines.push('ПЕРВАЯ фраза обязана содержать минимум один якорь из семени: имя/событие/платформа/чарт/число словами.');
    lines.push('Если в тексте нет якоря из семени — ответ считается браком.');
    if (narratorId === 'contemporary') {
      lines.push('Для СОВРЕМЕННИКА: после факта добавь контекст времени ТРЕКА (эпоха/сцена), не выдумывай «я там был».');
    }
