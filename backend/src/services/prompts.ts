...
ЗАПРЕЩЕНО (вода): «мало кто знает», «стала легендой», «зал славы», «суть в том что», «трогает сердца», «заслуженное место», «история о том как», «жанровая механика», «уникальный звук», «глубокий смысл», «настоящий прорыв» — без конкретики из семени.
- Каждое предложение = факт из семени (имя, событие, платформа, инструмент). Абзац про жанр без детали = БРАК.
...
export function buildStoryUserPrompt(params: {
  ...
  if (selected) {
    ...
    if (narratorId === 'contemporary') {
      ...
      lines.push(
        'СОВРЕМЕННИК: «лета 2014» или «в 2014 году» — нормально в тексте; не добавляй «тогда» сразу после года.',
      );
    }
    if (narratorId === 'night_dj') {
      lines.push(
        'НОЧНОЙ ДИДЖЕЙ: первое предложение — факт из семени (стримы, чарт, релиз), не поэтическая картинка.',
      );
      lines.push(
        'НОЧНОЙ ДИДЖЕЙ: без выдуманной студии, гитары, «летнего бриза» — только то, что в семени.',
      );
    }
    if (narratorId === 'fan') {
      lines.push(
    ...
    if (narratorId === 'expert') {
      lines.push(
        'Для ЭКСПЕРТА: жанр — одним словом; остальное только факты из семени. Не лекция о «жанровой механике».',
      );
    }
